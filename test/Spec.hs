...
import           Test.Hspec               (Spec, around, describe, hspec, it,
                                           pending, runIO, shouldBe)

import           Database                 (DbErr (..), Person (..),
                                           PersonNoId (..))
import           DatabaseEff              (DatabaseEff, databaseEffToIO,
...
import           Api                      (Routes (..), api, app, handleErrors,
                                           record)
...
withPersonApi action = do
  ...
  runM .
    runError @DbErr .
    runLogAction @IO logMessageStdout . runReader dbName . databaseEffToIO $
    makeTablesIfNotExists
  app' <- app dbName
